...
userSchema.methods.getDecryptedNationalId = function (this: IUser) {
  if (!this.nationalIdEncrypted) return null;
  try {
    return decrypt(this.nationalIdEncrypted);
  } catch (_err) {
    return null;
...
userSchema.pre<IUser>('save', async function (next) {
  ...
  if (this.isModified('nationalIdEncrypted') && this.nationalIdEncrypted) {
    if (!this.nationalIdEncrypted.includes(':')) {
      ...
      const nidHash = crypto.createHash('sha256')
        .update(this.nationalIdEncrypted, 'utf8')
        .digest('hex');
      ...
      if (!this.nationalIdHash) {
        try {
          ...
          this.nationalIdHash = crypto.createHash('sha256')
            .update(plain, 'utf8')
            .digest('hex');
        } catch (_err) {
